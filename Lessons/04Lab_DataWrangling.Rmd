...
NTL.pys.data.1999 <- filter(NTL.phys.data, year(sampledate) == 1999) 

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.pys.data.Tuesday <- filter(NTL.phys.data, year(sampledate) %in% c(1990:1999) & lakename == "Tuesday Lake") 

``` 

Question: Why don't we filter using row numbers?

> Answer: Row numbers change when adding new data or moving data around - not a reproducible data flow.
...
NTL.phys.data.Tues.processed <- 
  NTL.phys.data |> 
  filter(lakename == "Tuesday Lake") |> 
  filter(year(sampledate) %in% c(1990:1999) & month(sampledate) == 7) |> 
  select(lakename, sampledate:temperature_C) |> 
  mutate(temperature_F = (temperature_C*9/5) + 32)
...
NTL_Tues_surface <-
  ...
  #NTL_Tues_surface$MeanTempC <- mean(NTL_Tues_surface$temperature_C, na.rm = TRUE)
  #NTL_Tues_surface$MeanTempF <- mean(NTL_Tues_surface$temperature_F, na.rm = TRUE)
